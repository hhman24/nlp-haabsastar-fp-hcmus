...
#generator
def generator(l2):
    random_input = tf.random.uniform(shape=[FLAGS.batch_size, FLAGS.gen_input_dims])
    ...
    with tf.compat.v1.variable_scope('var_G') as scope:
        # defining weights and biases
        gen_h1 = tf.compat.v1.get_variable(name='gen_h1', shape=[n_input, n_hidden_1],
                                 initializer=tf.compat.v1.random_uniform_initializer(minval=-FLAGS.random_base,
                                                                           maxval=FLAGS.random_base),
                                 regularizer=tf.keras.regularizers.l2(0.5 * (l2)),
                                 dtype=tf.float32)

        gen_h2 = tf.compat.v1.get_variable(name='gen_h2', shape=[n_hidden_1, n_hidden_2],
                                 initializer=tf.compat.v1.random_uniform_initializer(minval=-FLAGS.random_base,
                                                                           maxval=FLAGS.random_base),
                                 regularizer=tf.keras.regularizers.l2(0.5 * (l2)),
                                 dtype=tf.float32)

        gen_h_out = tf.compat.v1.get_variable(name='gen_h_out', shape=[n_hidden_2, n_outputs],
                                    initializer=tf.compat.v1.random_uniform_initializer(minval=-FLAGS.random_base,
                                                                              maxval=FLAGS.random_base),
                                    regularizer=tf.keras.regularizers.l2(0.5 * (l2)),
                                    dtype=tf.float32)

        gen_b1 = tf.compat.v1.get_variable(name='gen_b1', shape=[n_hidden_1],
                                 initializer=tf.compat.v1.zeros_initializer(),
                                 regularizer=tf.keras.regularizers.l2(0.5 * (l2)),
                                 dtype=tf.float32)

        gen_b2 = tf.compat.v1.get_variable(name='gen_b2', shape=[n_hidden_2],
                                 initializer=tf.compat.v1.zeros_initializer(),
                                 regularizer=tf.keras.regularizers.l2(0.5 * (l2)),
                                 dtype=tf.float32)

        gen_b_out = tf.compat.v1.get_variable(name='gen_b_out', shape=[n_outputs],
                                    initializer=tf.compat.v1.zeros_initializer(),
                                    regularizer=tf.keras.regularizers.l2(0.5 * (l2)),
                                    dtype=tf.float32)
...
def lcr_rot (input_fw, input_bw, sen_len_fw, sen_len_bw, target, sen_len_tr, keep_prob1, keep_prob2, l2, _id='all'):
    ...
    # left hidden
    input_fw = tf.nn.dropout(input_fw, rate=1 - (keep_prob1),seed=1)
    hiddens_l = bi_dynamc_rnn(cell, input_fw, FLAGS.n_hidden, sen_len_fw, FLAGS.max_sentence_len, 'var_D' + '_lh', 'all')
    pool_l = reduce_mean_with_len(hiddens_l, sen_len_fw)

    # right hidden
    input_bw = tf.nn.dropout(input_bw, rate=1 - (keep_prob1),seed=2)
    ...
    # target hidden
    target = tf.nn.dropout(target, rate=1 - (keep_prob1),seed=3)
...
def discriminator(l, r, t_l, t_r, keep_prob2, l2):
    ...
    with tf.compat.v1.variable_scope('var_D') as scope:
        w_discriminator = tf.compat.v1.get_variable(
            name='discriminator_w',
            shape=[8*FLAGS.n_hidden,FLAGS.n_class],
            initializer=tf.compat.v1.random_uniform_initializer(-FLAGS.random_base, FLAGS.random_base),
            regularizer=tf.keras.regularizers.l2(0.5 * (l2))
        )
        b_discriminator = tf.compat.v1.get_variable(
    ...
    outputs = tf.nn.dropout(outputs_fin, rate=1 - (keep_prob2),seed=1234)
...
def main(train_path, test_path, accuracyOnt, test_size, remaining_size, learning_rate_dis, learning_rate_gen,
    ...
    with tf.device('/gpu:0'):
        ...
        with tf.compat.v1.name_scope('inputs'):
            x_real = tf.compat.v1.placeholder(tf.int32, [None, FLAGS.max_sentence_len])
            y_real = tf.compat.v1.placeholder(tf.float32, [None, FLAGS.n_class])
        ...
        inputs_fw = tf.nn.embedding_lookup(params=word_embedding, ids=x_real)
        ...
        with tf.compat.v1.variable_scope("var_D", reuse=tf.compat.v1.AUTO_REUSE) as scope:      #re-using the discriminator parameters since it is called twice per iter
        ...
        #set variable lists
        var_list_D = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope='var_D')
        var_list_G = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope='var_G')

        #As we solve a min max problem, we optimize twice with respect to different variable sets , var_list = var_D , var_list = var_G
        opti_min = tf.compat.v1.train.MomentumOptimizer(learning_rate=learning_rate_dis, momentum=momentum_dis).minimize(loss, var_list = var_list_D,
                                                                                                        global_step=global_step)
        opti_max = tf.compat.v1.train.MomentumOptimizer(learning_rate=learning_rate_gen, momentum=momentum_gen).minimize(-loss, var_list = var_list_G)

        true_y = tf.argmax(input=y_real, axis=1)
        pred_y = tf.argmax(input=prob_real, axis=1)
            ...
            for test, num in get_batch_data(te_x, te_sen_len, te_x_bw, te_sen_len_bw, te_y,
                ...
                if FLAGS.method == 'TD-ATT' or FLAGS.method == 'IAN':
                    _loss, _acc, _fw, _bw, _tl, _tr, _ty, _py, _p = sess.run(
                        [loss, acc_num_gen, alpha_fw, alpha_bw, alpha_t_l, alpha_t_r, true_y, pred_y, prob_real], feed_dict=test)
                    fw += list(_fw)
                    bw += list(_bw)
                    tl += list(_tl)
                    tr += list(_tr)
