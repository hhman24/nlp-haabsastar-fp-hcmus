...
def main(_):
    ...
    runLCRROTALT = False
    runSVM = False
    runLCRModelAlt_hierarchical_v4 = False
    runAdversarial = True
    ...
    REMAIN_val = FLAGS.hardcoded_path+"src/data/programGeneratedData/crossValidation"+str(FLAGS.year)+'/cross_val_remainder_'
    REMAIN_svm_val = FLAGS.hardcoded_path+"src/data/programGeneratedData/crossValidation"+str(FLAGS.year)+'/svm/cross_val_remainder_'
    ...
    if useOntology == True:
        ...
        with open(FLAGS.hardcoded_path+"src/data/programGeneratedData/crossValidation"+str(FLAGS.year)+'/cross_results_"+str(FLAGS.year)+"/ONTOLOGY_"+str(FLAGS.year)'+'.txt', 'w') as result:
    ...
    if runLCRROTALT == True:
        ...
        for i in range(split_size):
            ...
            tf.compat.v1.reset_default_graph()
            print('iteration: '+ str(i))
        with open(FLAGS.hardcoded_path+"src/data/programGeneratedData/crossValidation"+str(FLAGS.year)+'/cross_results_"+str(FLAGS.year)+"/LCRROT_ALT_"+str(FLAGS.year)'+'.txt', 'w') as result:
            result.write(str(acc))
            result.write('Accuracy: {}, St Dev:{} /n'.format(np.mean(np.asarray(acc)), np.std(np.asarray(acc))))
            print(str(split_size)+'-fold cross validation results')
            print('Accuracy: {}, St Dev:{}'.format(np.mean(np.asarray(acc)), np.std(np.asarray(acc))))
    ...
    if runLCRModelAlt_hierarchical_v4 == True:
        ...
        acc.append(acc1)
        tf.compat.v1.reset_default_graph()
        with open(FLAGS.hardcoded_path+"src/data/programGeneratedData/crossValidation"+str(FLAGS.year)+"cross_results_" + str(FLAGS.year) + "/v4_" + str(FLAGS.year) + '.txt', 'w') as result:
            print(str(split_size) + '-fold cross validation results')
        print('Accuracy: {:.5f}, St Dev:{:.4f}'.format(np.mean(np.asarray(acc)), np.std(np.asarray(acc))))
        result.write(str(acc))
        result.write('Accuracy: {:.5f}, St Dev:{:.4f} /n'.format(np.mean(np.asarray(acc)), np.std(np.asarray(acc))))
    ...
    if runAdversarial == True:
        ...
        acc.append(acc1)
        tf.compat.v1.reset_default_graph()
        with open(FLAGS.hardcoded_path+"src/data/programGeneratedData/crossValidation"+str(FLAGS.year)+"cross_results_" + str(FLAGS.year) + "/Adv_" + str(FLAGS.year) + '.txt', 'w') as result:
            print(str(split_size) + '-fold cross validation results')
        print('Accuracy: {:.5f}, St Dev:{:.4f}'.format(np.mean(np.asarray(acc)), np.std(np.asarray(acc))))
        result.write(str(acc))
        result.write('Accuracy: {:.5f}, St Dev:{:.4f} /n'.format(np.mean(np.asarray(acc)), np.std(np.asarray(acc))))
