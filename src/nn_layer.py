...
def cnn_layer(inputs, filter_size, strides, padding, random_base, l2_reg, active_func=None, scope_name="conv"):
    w = tf.compat.v1.get_variable(
        ...
        initializer=tf.compat.v1.random_uniform_initializer(-random_base, random_base),
    ...
    x = tf.nn.conv2d(input=inputs, filters=w, strides=strides, padding=padding) + b
...
def dynamic_rnn(cell, inputs, n_hidden, length, max_len, scope_name, out_type='last'):
    ...
    mask_3d = tf.reshape(mask_tiled, tf.shape(input=outputs))
...
def bi_dynamic_rnn(cell, inputs, n_hidden, length, max_len, scope_name, out_type='last'):
    ...
    if out_type == 'last':
        ...
        outputs_bw = tf.reverse_sequence(input=outputs_bw, seq_lengths=tf.cast(length, tf.int64), seq_axis=1)
...
def bi_dynamic_rnn_diff(cell, inputs_fw, inputs_bw, n_hidden, l_fw, l_bw, max_len, scope_name):
    with tf.compat.v1.name_scope('forward_lstm'):
    ...
    with tf.compat.v1.name_scope('backward_lstm'):
        ...
        batch_size = tf.shape(input=outputs_bw)[0]
...
def reduce_mean_with_len(inputs, length):
    ...
    inputs = tf.reduce_sum(input_tensor=inputs, axis=1, keepdims=False) / length
    return inputs


def softmax_layer(inputs, n_hidden, random_base, keep_prob, l2_reg, n_class, scope_name='1'):
    # fix bug
    # with tf.compat.v1.variable_scope('softmax_w' + scope_name, reuse=tf.compat.v1.AUTO_REUSE) as scope:
    w = tf.compat.v1.get_variable(
        name='softmax_w' + scope_name,
        shape=[n_hidden, n_class],
        initializer=tf.compat.v1.random_normal_initializer(mean=0., stddev=np.sqrt(2. / (n_hidden + n_class))),
        # initializer=tf.random_uniform_initializer(-random_base, random_base),
        # initializer=tf.random_uniform_initializer(-np.sqrt(6.0 / (n_hidden + n_class)), np.sqrt(6.0 / (n_hidden + n_class))),
        regularizer=tf.keras.regularizers.l2(0.5 * (l2_reg))
    )

    # with tf.compat.v1.variable_scope('softmax_b' + scope_name, reuse=tf.compat.v1.AUTO_REUSE) as scope:
    b = tf.compat.v1.get_variable(
        name='softmax_b' + scope_name,
        shape=[n_class],
        initializer=tf.compat.v1.random_normal_initializer(mean=0., stddev=np.sqrt(2. / (n_class))),
        # initializer=tf.random_uniform_initializer(-random_base, random_base),
        regularizer=tf.keras.regularizers.l2(0.5 * (l2_reg))
    )

    with tf.compat.v1.name_scope('softmax'):
        outputs = tf.nn.dropout(inputs, rate=1 - (keep_prob))
